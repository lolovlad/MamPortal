...
from ..service import LoginServices
...
@router.post("/registrate", response_model=Token, responses={
    status.HTTP_406_NOT_ACCEPTABLE: {"model": Message}
})
async def registrate(request: Request,
                     ...
                     login_services: LoginServices = Depends()):
    try:
        token = await login_services.registrate_user(target)
        return token
    except Exception:
        return JSONResponse(content={"message": "Ошибка регистрации"},
                            status_code=status.HTTP_406_NOT_ACCEPTABLE)