...
from ..tables import User, TypeUser as TypeUserORM
...
class UserRepository:
    ...
    async def get_limit_user(self, start: int, end: int) -> list[User]:
        response = select(User).where(User.is_deleted == False).offset(start).limit(end).order_by(User.id)
        result = await self.__session.execute(response)
    ...
    async def get_user_by_uuid(self, uuid: str) -> User:
        response = select(User).where(User.uuid == uuid).where(User.is_deleted == False)
        result = await self.__session.execute(response)
    ...
    async def get_users_by_search_field(self,
        ...
        )).where(User.is_deleted == False).limit(count).order_by(User.id)
        result = await self.__session.execute(response)
        return result.scalars().all()
