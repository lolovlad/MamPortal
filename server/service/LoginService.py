...
from ..models.User import UserGet, TypeUser, UserPost
...
class LoginServices:
    ...
    async def registrate_user(self, target: UserPost) -> Token:
        user_type = [i for i in await self.__repo.get_all_type_user() if i.name == "user"][0]
        ...
        entity.password = target.password
        try:
            await self.__repo.add(entity)
        except Exception:
            raise Exception

        token = self.create_token(entity)
        return token
